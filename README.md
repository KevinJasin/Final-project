# Veebileht eriõpetajale

## Ülevaade
See projekt hõlmab veebilehe loomist eriõpetajale, mille peamiseks eesmärgiks on pakkuda klientidele võimalust broneerida aega ning hallata oma andmeid. Veebileht sisaldab kalendrit ja andmebaasi, kus hoitakse klientide kontakte.

## Funktsionaalsus

### 1. Kalender
- Aegade broneerimine
- Broneeringute haldamine
- Teavitused (kinnitused ja meeldetuletused)

### 2. Andmebaas
- Klientide andmed (nimi, telefoninumber, e-post)
- Andmete haldamine (lisamine, muutmine, kustutamine)

### 3. Kasutajaliides
- Mugav ja intuitiivne disain
- Kontaktvorm
- Ülevaade broneeringutest

## Tehnilised nõuded
- **Veebitehnoloogiad:** 
- **Andmebaas:** 
- **Raamistik:** 

## Ajakava

| Aeg        | Tegevus                                          | Vastutav isik |
|------------|--------------------------------------------------|----------------|
| 1. nädal   | Projekti planeerimine ja nõuete kogumine        | Klient ja Arendaja       |
| 2. nädal   | Disaini loomine ja prototüüpide koostamine      | Disainer       |
| 3. nädal   | Andmebaasi struktuuri loomine                   | Arendaja       |
| 4. nädal   | Kalendri ja broneerimise funktsionaalsuse arendamine | Arendaja       |
| 5. nädal   | Andmebaasi ja kasutajaliidese integreerimine    | Arendaja       |
| 6. nädal   | Testimine ja tagasiside kogumine                | Klient ja Arendaja       |
| 7. nädal   | Lõpetamine ja veebilehe käivitamine             | Arendaja      |
| 8. nädal   | Jätkusuutlikkuse ja hoolduse plaanimine          | Klient ja Arendaja       |

## Järgmised sammud
- Kohtumine disaineriga, et arutada disaini.
- Koostada detailne tehniline dokumentatsioon.
- Alustada arendustöid vastavalt ajakavale.

---

